...
export default function VideoTraining({className, imgBtn, text}) {
    ...
    // bikin if else kalo ada id maka setData response
    // try catch dibagian edit data
    useEffect(() => {
    ...
    const handleChange = (e) => {
        ...
        if(name === "video"){
            ...
            if(videoValue && videoValue.type.match('video/*')){
                const storageRef = ref(storage, `/files/${videoValue.name}`)
                const uploadImg = uploadBytesResumable(storageRef, videoValue)
                uploadImg.on(
                    'state_Changed',
                    (snapshot) => {
